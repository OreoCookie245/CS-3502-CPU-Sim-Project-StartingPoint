...
4. Compare algorithm behaviors and outcomes";
        ...
        /// </summary>
        /// <summary>
        /// Shortest Remaining Time First - preemptive (FIXED VERSION)
        /// </summary>
        private List<SchedulingResult> RunSRTFAlgorithm(List<ProcessData> processes)
        {
            ...
            while (completed < n)
            {
                ...
                if (!isStarted[selectedIdx])
                {
                    startTime[selectedIdx] = currentTime;
                    isStarted[selectedIdx] = true;
                }

                // Execute for 1 time unit
                remainingTime[selectedIdx]--;
                currentTime++;

                // Check if process completed
                if (remainingTime[selectedIdx] == 0)
                {
                    finishTime[selectedIdx] = currentTime;
                    isCompleted[selectedIdx] = true;
                    completed++;

                    results.Add(new SchedulingResult
                    {
                        ProcessID = processes[selectedIdx].ProcessID,
                        ArrivalTime = processes[selectedIdx].ArrivalTime,
                        BurstTime = processes[selectedIdx].BurstTime,
                        StartTime = startTime[selectedIdx],
                        FinishTime = finishTime[selectedIdx],
                        WaitingTime = finishTime[selectedIdx] - processes[selectedIdx].ArrivalTime - processes[selectedIdx].BurstTime,
                        TurnaroundTime = finishTime[selectedIdx] - processes[selectedIdx].ArrivalTime
                    });
                }
            }
        ...
        /// </summary>
        /// <summary>
        /// Highest Response Ratio Next - non-preemptive (FIXED VERSION)
        /// </summary>
        private List<SchedulingResult> RunHRRNAlgorithm(List<ProcessData> processes)
        {
            ...
            var remainingProcesses = processes.ToList();
            var completed = new bool[processes.Count];

            while (remainingProcesses.Count > 0)
            {
                // Get available processes that have arrived
                var available = remainingProcesses
                    .Where(p => p.ArrivalTime <= currentTime && !completed[processes.IndexOf(p)])
                    .ToList();
                ...
                if (available.Count == 0)
                {
                    ...
                    currentTime = remainingProcesses
                        .Where(p => !completed[processes.IndexOf(p)])
                        .Min(p => p.ArrivalTime);
                    continue;
                }

                // Calculate response ratio for each available process
                var processWithRatios = available.Select(p => new
                {
                    Process = p,
                    ResponseRatio = (currentTime - p.ArrivalTime + p.BurstTime) / (double)p.BurstTime
                }).ToList();

                // Select process with highest response ratio
                var selected = processWithRatios
                    .OrderByDescending(p => p.ResponseRatio)
                    .ThenBy(p => p.Process.ArrivalTime)
                    .First();
                ...
                double finishTime = startTime + process.BurstTime;
                double waitingTime = startTime - process.ArrivalTime;
                ...
                results.Add(new SchedulingResult
                {
                    ...
                    TurnaroundTime = (int)turnaroundTime
                });
        ...
        private void DisplaySchedulingResults(List<SchedulingResult> results, string algorithmName)
        {
            ...
            double avgWaitingTime = results.Average(p => p.WaitingTime);
            double avgTurnaroundTime = results.Average(p => p.TurnaroundTime);
            double totalBurstTime = results.Sum(p => p.BurstTime);
            double totalTime = results.Max(p => p.FinishTime);
            ...
            summaryItem.SubItems.Add($"AWT: {avgWaitingTime:F2}");
            summaryItem.SubItems.Add($"ATT: {avgTurnaroundTime:F2}");
            summaryItem.SubItems.Add($"");
            summaryItem.SubItems.Add($"");
            listView1.Items.Add(summaryItem);

            var metricsItem = new ListViewItem("METRICS");
            metricsItem.SubItems.Add($"CPU: {cpuUtilization:F1}%");
            metricsItem.SubItems.Add($"Throu: {throughput:F3}/t");
            metricsItem.SubItems.Add($"RT: {avgResponseTime:F2}");
        ...
        /// <summary>
        /// STUDENTS: Export results to CSV for project report analysis
        /// </summary>
        private void ExportResultsToCSV(List<SchedulingResult> results, string algorithmName)
        {
            using (var saveDialog = new SaveFileDialog())
            {
                ...
                saveDialog.Title = "Export Results for Project Report";
                ...
                if (saveDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        // Write header
                        writer.WriteLine("Algorithm,ProcessID,ArrivalTime,BurstTime,StartTime,FinishTime,WaitingTime,TurnaroundTime");

                        // Write data
                        foreach (var result in results)
                        {
                            writer.WriteLine($"{algorithmName},{result.ProcessID},{result.ArrivalTime},{result.BurstTime},{result.StartTime},{result.FinishTime},{result.WaitingTime},{result.TurnaroundTime}");
                        }

                        // Write summary
                        writer.WriteLine();
                        writer.WriteLine("METRICS SUMMARY");
                        writer.WriteLine($"Average Waiting Time,{results.Average(r => r.WaitingTime):F2}");
                        writer.WriteLine($"Average Turnaround Time,{results.Average(r => r.TurnaroundTime):F2}");
                        writer.WriteLine($"CPU Utilization,{(results.Sum(r => r.BurstTime) / (double)results.Max(r => r.FinishTime)) * 100:F1}%");
                        writer.WriteLine($"Throughput,{results.Count / (double)results.Max(r => r.FinishTime):F3}");
                    }
        ...
        private void FirstComeFirstServeButton_Click(object sender, EventArgs e)
        {
            ...
            if (processData.Count > 0)
            {
                ...
                ExportResultsToCSV(results, "FCFS");
        ...
        private void ShortestJobFirstButton_Click(object sender, EventArgs e)
        {
            ...
            if (processData.Count > 0)
            {
                ...
                ExportResultsToCSV(results, "SJF");
        ...
        private void PriorityButton_Click(object sender, EventArgs e)
        {
            ...
            if (processData.Count > 0)
            {
                ...
                ExportResultsToCSV(results, "Priority");
        ...
        private void ApplyLightTheme()
        {
            ...
            btnSRTF.BackColor = Color.PeachPuff;
            btnHRRN.BackColor = Color.LightSalmon;
